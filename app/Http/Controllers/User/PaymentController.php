...
{   
    ...
    public function pay(Request $request, SubscriptionPlan $id)
    {
        if ($id->free) {
            ...
            $paymentName = strtolower($this->paymentPlatforms->firstWhere('id', $request->payment_platform)->name);

            if ($paymentName == "paytr") {
                $products = [
                    [
                        'Subscription',
                        60.00, // Product Price
                ...
                $payment = \Paytr::payment()  
                    ...
                    ->setUserPhone("12321423123")  
                    ->setUserAddress($request->user()->address.",".$request->user()->city.",".$request->user()->country)  
                    ->setNoInstallment(1)  
                    ->setMaxInstallment(1)  
                    ->setEmail($request->user()->email)  
                    ->setMerchantOid(1) // Payment id generated by you 
                    ->setUserIp($request->ip())  
                    ->setPaymentAmount($id->price * 1)  // Total payment amount
